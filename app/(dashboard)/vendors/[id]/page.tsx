    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        <div>
          ...
          {vendor.businessName && (
            <p className="text-gray-500 mt-1">{vendor.businessName}</p>
          )}
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
            <h3 className="text-sm font-bold text-gray-900">Tax Information</h3>
          </div>
          <div className="p-6 space-y-4 text-sm text-gray-700">
            <div>
              <span className="block text-gray-500 text-xs mb-1">GSTIN</span>
              <span className="font-medium">{vendor.gstin || "Not provided"}</span>
            </div>
            <div>
              <span className="block text-gray-500 text-xs mb-1">PAN</span>
          ...
          <div className="p-6 text-sm text-gray-700">
            {vendor.address ? (
              <div className="space-y-1">
                ...
                {vendor.stateCode && <p className="text-xs text-gray-500 mt-2">State Code: {vendor.stateCode}</p>}
              </div>
            ) : (
