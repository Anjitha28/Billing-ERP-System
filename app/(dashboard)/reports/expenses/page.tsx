    ...
    <div className="p-6 max-w-7xl mx-auto print:p-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-red-500 print:shadow-none print:border print:border-gray-300">
          <h3 className="text-xs font-medium text-gray-500 uppercase">Total Expenses</h3>
          <p className="mt-1 text-xl font-bold text-gray-900">{formatCurrency(summary.totalExpenses)}</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-purple-500 print:shadow-none print:border print:border-gray-300">
          <h3 className="text-xs font-medium text-gray-500 uppercase">Total TDS</h3>
          <p className="mt-1 text-xl font-bold text-gray-900">{formatCurrency(summary.totalTDS)}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-green-500 print:shadow-none print:border print:border-gray-300">
          <h3 className="text-xs font-medium text-gray-500 uppercase">Paid Expenses</h3>
          <p className="mt-1 text-xl font-bold text-gray-900">{formatCurrency(summary.paidExpenses)}</p>
        </div>
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-orange-500 print:shadow-none print:border print:border-gray-300">
          <h3 className="text-xs font-medium text-gray-500 uppercase">Unpaid Expenses</h3>
          <p className="mt-1 text-xl font-bold text-gray-900">{formatCurrency(summary.unpaidExpenses)}</p>
        </div>
      </div>
        ...
        <div className="p-4 border-b border-gray-200 bg-gray-50 no-print">
          <form className="flex flex-col md:flex-row gap-3">
            ...
            {(searchFilter || statusFilter || fromFilter || toFilter) && (
              <Link href="/reports/expenses" className="bg-white text-gray-500 px-4 py-2 rounded-lg text-sm font-medium hover:text-gray-700 border border-gray-300 flex items-center justify-center">Clear</Link>
            )}
          </form>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50 print:bg-white print:border-b-2 print:border-gray-800">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Input GST</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">TDS</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {expenses.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-4 py-8 text-center text-sm text-gray-500">No expenses found for the selected criteria.</td>
                </tr>
              ) : (
                expenses.map((exp) => (
                  <tr key={exp.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900 max-w-[150px] truncate">
                      ...
                      <div className="text-xs text-gray-500">{exp.category?.name || ''}</div>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-right text-gray-500">{formatCurrency(Number(exp.taxableAmount))}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-right text-gray-500">{formatCurrency(Number(exp.totalInputGST))}</td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-center">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${
                        ...
                        exp.paymentStatus === 'PARTIALLY_PAID' ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800'
