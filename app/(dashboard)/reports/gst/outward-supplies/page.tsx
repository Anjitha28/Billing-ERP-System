  ...
  return (
    <div className="p-6 max-w-7xl mx-auto print:p-0">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 no-print">
        <h1 className="text-2xl font-bold text-theme-text">GST Outward Supplies</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8">
        <div className="bg-theme-surface rounded-lg shadow p-4 border-l-4 border-theme-primary print:shadow-none print:border print:border-theme-border">
          <h3 className="text-xs font-medium text-theme-text-muted uppercase">Total Taxable Value</h3>
          <p className="mt-1 text-xl font-bold text-theme-text">{formatCurrency(summary.totalTaxableValue)}</p>
        </div>
        <div className="bg-theme-surface rounded-lg shadow p-4 border-l-4 border-indigo-500 print:shadow-none print:border print:border-theme-border">
          <h3 className="text-xs font-medium text-theme-text-muted uppercase">Total CGST</h3>
          <p className="mt-1 text-xl font-bold text-theme-text">{formatCurrency(summary.totalCGST)}</p>
        </div>
        <div className="bg-theme-surface rounded-lg shadow p-4 border-l-4 border-purple-500 print:shadow-none print:border print:border-theme-border">
          <h3 className="text-xs font-medium text-theme-text-muted uppercase">Total SGST</h3>
          <p className="mt-1 text-xl font-bold text-theme-text">{formatCurrency(summary.totalSGST)}</p>
        </div>
        <div className="bg-theme-surface rounded-lg shadow p-4 border-l-4 border-pink-500 print:shadow-none print:border print:border-theme-border">
          <h3 className="text-xs font-medium text-theme-text-muted uppercase">Total IGST</h3>
          <p className="mt-1 text-xl font-bold text-theme-text">{formatCurrency(summary.totalIGST)}</p>
        </div>
        <div className="bg-theme-surface rounded-lg shadow p-4 border-l-4 border-green-500 print:shadow-none print:border print:border-theme-border">
          <h3 className="text-xs font-medium text-theme-text-muted uppercase">Total Output GST</h3>
          <p className="mt-1 text-xl font-bold text-theme-text">{formatCurrency(summary.totalOutputGST)}</p>
        </div>
      </div>
          ...
          <table className="min-w-full divide-y divide-theme-border">
            <thead className="bg-theme-surface-hover print:bg-theme-surface print:border-b-2 print:border-gray-800">
              <tr>
                ...
                <th className="px-4 py-3 text-right text-xs font-medium text-theme-text-muted uppercase tracking-wider">CGST</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-theme-text-muted uppercase tracking-wider">SGST</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-theme-text-muted uppercase tracking-wider">IGST</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-theme-text-muted uppercase tracking-wider">Total GST</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-theme-text-muted uppercase tracking-wider">Invoice Value</th>
              </tr>
            </thead>
            <tbody className="bg-theme-surface divide-y divide-theme-border">
              {invoices.length === 0 ? (
                <tr>
                  <td colSpan={9} className="px-4 py-8 text-center text-sm text-theme-text-muted">No GST outward supplies found for the selected criteria.</td>
                </tr>
              ) : (
                invoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-theme-surface-hover">
                    <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-theme-primary">
                      ...
                      <div className="text-xs text-theme-text-muted">{inv.invoiceDate.toLocaleDateString('en-IN')}</div>
                    </td>
                    <td className="px-4 py-3 text-sm text-theme-text max-w-[200px] truncate">
                      ...
                      <div className="text-xs text-theme-text-muted font-mono">{inv.gstinSnapshot || inv.customer?.gstin || ''}</div>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-theme-text-muted">{inv.stateSnapshot || inv.customer?.state || ''}</td>
