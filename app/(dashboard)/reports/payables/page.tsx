    ...
    <div className="p-6 max-w-7xl mx-auto print:p-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-red-500 print:shadow-none print:border print:border-gray-300">
          <h3 className="text-xs font-medium text-gray-500 uppercase">Total Payables</h3>
          <p className="mt-1 text-xl font-bold text-gray-900">{formatCurrency(summary.totalPayables)}</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow p-4 border-l-4 border-blue-500 print:shadow-none print:border print:border-gray-300">
          ...
          <p className="mt-1 text-xl font-bold text-gray-900">{summary.numberOfUnpaidExpenses}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow border border-gray-100 overflow-hidden print:shadow-none print:border-0">
        <div className="p-4 border-b border-gray-200 bg-gray-50 no-print">
          <form className="flex flex-col md:flex-row gap-3">
            ...
            {(fromFilter || toFilter) && (
              <Link href="/reports/payables" className="bg-white text-gray-500 px-4 py-2 rounded-lg text-sm font-medium hover:text-gray-700 border border-gray-300 flex items-center justify-center">Clear</Link>
            )}
          </form>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {expenses.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-sm text-gray-500">No outstanding payables found.</td>
                </tr>
              ) : (
                expenses.map((exp) => (
                  <tr key={exp.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-right text-gray-900">{formatCurrency(Number(exp.netAmount))}</td>
